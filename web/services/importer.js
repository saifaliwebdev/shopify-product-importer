...
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix nonuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix decuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix undecuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix duodecuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix tredecuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix quattuordecuple <br> tags
//           ?.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>\s*<br\s*\/?>/g, '<br>') // Fix quindecuple <br> tags
...
class ProductImporter {
  ...
  async importProduct(session, productData, options = {}) {
    ...
    try {
      ...
      const variants = this.applyPriceMarkup(productData.variants, priceMarkup, priceMarkupType);
      console.log("📦 Variants count:", variants.length);

      // Status as GraphQL enum
      const productStatus = status === "active" ? "ACTIVE" : "DRAFT";
      ...
      const productInput = {
        ...
        descriptionHtml: productData.description
          ...
          ?.trim() || "<p>Product description</p>",
        vendor: finalVendor || "Imported Product",
        status: productStatus, // enum ACTIVE/DRAFT
        productType: productData.product_type?.trim() || "General",
      };

      // Add tags only if non-empty
      if (Array.isArray(productData.tags) && productData.tags.length > 0) {
        productInput.tags = productData.tags.map(t => String(t).trim()).filter(t => t.length > 0);
      }

      // Add options only if valid
      if (productData.options && productData.options.length > 0) {
        const validOptions = productData.options.map(opt => ({
          name: String(opt.name).trim(),
          values: opt.values.map(v => String(v).trim()).filter(v => v.length > 0)
        })).filter(opt => opt.values.length > 0);

        if (validOptions.length > 0) {
          productInput.options = validOptions;
        }
      }
      ...
      if (result?.userErrors?.length > 0) {
        result.userErrors.forEach(error => {
          console.error(`❌ Error in field: ${error.field ? error.field.join('.') : 'unknown'} - ${error.message}`);
        });
        throw new Error(result.userErrors.map(e => e.message).join(", "));
      }

      if (!result?.product) throw new Error("Product creation failed - no product returned");
      ...
      if (variants.length <= 1 && (!productData.options || productData.options.length === 0)) {
        ...
        await this.createAllVariants(client, productId, variants, createdProduct.options || [], inventoryQuantity);
      }
